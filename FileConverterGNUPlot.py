import csv
import sys
""" The following script converts a node file to five output files, for 'time-stamp vs specificity', 
'time-stamp vs diversity', 'time-stamp vs entropy', 'time-stamp vs pielou', and the last one with
'time-stamp vs all the above mentioned properties'. These files are needed for gnuplot script to plot them. """
    ...
    with open(infilename + '-specificity.txt', 'w') as specificity_csv, \
        ...
        with open(input_file) as f:
            ...
            while line:  # This while loop is to read one line at a time until the lines are finished
                ...
                line = f.readline()
                date_stamp = current_line[1][:-10]  # Save Date only
                time_stamp = current_line[1][11:-1]  # Save the time only
                ...
                pielou = [str(timestamp), float(current_line[6])]
...
# InFile = 'output_files/foobar-nodes.csv'
InFile = sys.argv[1]
